package com.darbin.ontu.custom.edittext
...
class CustomEditText @JvmOverloads constructor(
    ...
    private var animationDuration = 50
    ...
    private fun initAttrs(attributeSet: AttributeSet) {
        with(
            context.obtainStyledAttributes(
                attributeSet,
                R.styleable.CustomEditTextStylable,
                defStyleRes,
                0
            )
        ) {
            paint.color = getColor(
                R.styleable.CustomEditTextStylable_borderColor,
                paint.color
            )
            paint.strokeWidth = getDimension(
                R.styleable.CustomEditTextStylable_borderWidth,
                paint.strokeWidth
            )
            collapsedTextSize = getDimension(
                R.styleable.CustomEditTextStylable_animatedTextSize,
                paint.textSize
            ).also {
                paint.textSize = it
            }
            cornerRadius = getDimension(
                R.styleable.CustomEditTextStylable_cornerRadius,
                cornerRadius
            )
            animationDuration = getInteger(
                R.styleable.CustomEditTextStylable_animationDuration,
                animationDuration
            )
        }
    }
